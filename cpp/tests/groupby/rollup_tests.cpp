...
{
  cudf::groupby::groupby roller(keys, cudf::null_policy::INCLUDE, cudf::sorted::NO, {}, {});
  std::vector<cudf::groupby::aggregation_request> requests(1);
  requests[0].values = values;
  requests[0].aggregations.push_back(cudf::make_sum_aggregation<cudf::groupby_aggregation>());
  return roller.rollup(rolled_indices, requests, cudf::test::get_default_stream());
}

/**
 * Output row order from the hash path is not fixed; sort by the INT64 group_id column, then keys
 * (nulls last), then sum.
 */
  ...
  cudf::table_view const& keys_and_group_id, cudf::column_view const& sum_col)
{
  ...
  sort_keys.push_back(keys_and_group_id.column(nk));
  for (cudf::size_type c = 0; c < nk; ++c) {
    sort_keys.push_back(keys_and_group_id.column(c));
  }
...
// -----------------------------------------------------------------------------
// Host reference for ROLLUP + SUM (null_policy::INCLUDE on keys).
// -----------------------------------------------------------------------------
...
/** Bitmask for grouping level `g`: (1 << g) - 1, matching the rollup output group_id column. */
[[nodiscard]] int64_t rollup_group_id_for_level(cudf::size_type grouping_level)
{
  ...
  cudf::size_type num_key_cols)
{
  ...
  cudf::test::fixed_width_column_wrapper<int64_t> w_group_id(e_group_id.begin(), e_group_id.end());
    ...
    expected_rows)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(sorted_gpu->get_column(2), w_group_id);
                                                          ...
                                                          std::optional<SumT>>>& out_sorted)
{
  ...
  for (cudf::size_type r = 0; r < n; ++r) {
    for (cudf::size_type level = 0; level < nlev; ++level) {
      AccKey key{rollup_group_id_for_level(level), std::nullopt, std::nullopt, std::nullopt};
    ...
    out_sorted)
{
  ...
  for (cudf::size_type r = 0; r < n; ++r) {
    ...
    for (cudf::size_type level = 0; level < nlev; ++level) {
      ...
      AccKey key{rollup_group_id_for_level(level), p0, p1};
      acc[key] += vals[r];
    }
  }
  out_sorted.clear();
...
TEST_F(RollupTest, OneIntKeySumSingleInputRow)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(sorted->get_column(1), expect_group_id);
...
TEST_F(RollupTest, TwoIntKeysSumRollupBoth)
{
  ...
  cudf::test::fixed_width_column_wrapper<int64_t> expect_group_id{0, 0, 1, 3};
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(sorted->get_column(2), expect_group_id);
...
TEST_F(RollupTest, EmptyRolledListOrdinaryGroupByWithGroupingId)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(sorted->get_column(1), expect_group_id);
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(sorted->get_column(2), expect_sum);
}

TEST_F(RollupTest, SingleRolledKeyProducesGrandTotalRowAndGroupIdMultiset)
{
  cudf::test::fixed_width_column_wrapper<int32_t> k{1, 2};
  cudf::test::fixed_width_column_wrapper<int32_t> v{10, 20};
  cudf::table_view const keys{{k}};
  ...
  EXPECT_EQ(3, out_keys_group_id->num_rows());
  EXPECT_EQ(2, out_keys_group_id->num_columns());
  EXPECT_EQ(1, out_keys_group_id->get_column(0).null_count());

  auto const group_id_view = out_keys_group_id->get_column(1).view();
  std::vector<cudf::column_view> const group_id_col_views{group_id_view};
...
{
  // SUM with a nullable measure column: null inputs do not contribute to the sum.
  constexpr int n = 5000;
  std::vector<int32_t> k0(static_cast<std::size_t>(n));
  std::vector<int32_t> k1(static_cast<std::size_t>(n));
  std::vector<sum_type> vals(static_cast<std::size_t>(n));
...
TEST_F(RollupTest, FusedKernelStyle_Int64KeysTwoRolledSum5000Rows)
{
  ...
  cudf::table_view const keys{{w0, w1}};
  std::vector<cudf::size_type> const rolled{0, 1};
  cudf::groupby::groupby roller(keys, cudf::null_policy::INCLUDE, cudf::sorted::NO, {}, {});
  std::vector<cudf::groupby::aggregation_request> requests(1);
