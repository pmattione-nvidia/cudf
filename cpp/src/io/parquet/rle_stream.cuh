...
struct rle_stream {
  ...
  __device__ inline int skip_runs(int target_count)
  {
    ...
    // amount so thread 0 spins like crazy on fill_run_batch(), skipping writing unnecessary run
    // info then when it hits the one that matters, we don't process it at all and bail as if we
    // never started basically we're setting up the global vars necessary to start fill_run_batch
    // for the first time
    while (cur < end) {
      // bytes for the varint header
      uint8_t const* _cur = cur;
  ...
  __device__ inline int skip_decode(int t, int count)
  {
    ...
    // warp 0 reads ahead and fills `runs` array to be decoded by remaining warps.
    if (t == 0) { values_processed_shared = skip_runs(output_count); }
    __syncthreads();
